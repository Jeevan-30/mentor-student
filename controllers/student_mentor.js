...
export const apiConnected = async (req, res) => {
  try {
    ...
    await res.status(200).json({
      message: `Mentor & Student API are Successfully Connected! Kindly check the API in Postman`,
    });
  } catch (error) {
    //throw error if anything goes wrong
    console.log(`There is a error in connecting to API : ${error}`);
...
export const assignStudentsToMentor = async (req, res) => {
  try {
    ...
    //sending success response
    res.status(200).json({
      message: "Students are successfully assigned to mentor!",
...
export const assignMentorToStudent = async (req, res) => {
  try {
    ...
    // Throw error if anything goes wrong
    console.error("Error while assigning the mentor:", error);
    res.status(500).json({ message: `Internal Server Error` });
  }
};
...
export const getStudentsByMentorID = async (req, res) => {
  try {
    ...
    //throw error if anything goes wrong
    console.error("Error while fetching mentor students details:", error);
    res.status(500).json({ message: "Internal Server Error" });
  }
};
